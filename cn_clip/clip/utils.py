...
def load_from_name(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", vision_model_name: str = None, text_model_name: str = None, input_resolution: int = None):
    if name in _MODELS:
        model_path = r"D:\Code\ppt_by_llm\ckpt\clip_cn_rn50.pt"
...
def load_from_path(name: str, model_path,device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu"):
    if name in _MODELS:
        model_name, model_input_resolution = _MODEL_INFO[name]['struct'], _MODEL_INFO[name]['input_resolution']
    else:
        raise RuntimeError(f"Model {name} not found; available models = {available_models()}")
